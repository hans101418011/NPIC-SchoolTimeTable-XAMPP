...
----------------------------------------------------*/
		...
		$serch_sect = array("1-國際貿易系(科)","2-企業管理系(科)","3-財務金融系(科)","6-資訊管理系");
		$serch_grade = array("1","2","3","4");

		$method="POST";
		$data_array = array("dept"=>"", "sect"=>"", "grade"=>"", "cscn"=>"", "thname"=>"", "dayinweek"=>"", "selnscode"=>"", "periods"=>"", "room"=>"");


		$data_array["dept"] = "7-夜四技";
		$data_array["dept"]=iconv("UTF-8","big5",$data_array["dept"]);

		for($sect_num=0;$sect_num<count($serch_sect);$sect_num++)
		{
			for($grade_num=0;$grade_num<count($serch_grade);$grade_num++)
			{
				$data_array["sect"] = $serch_sect[$sect_num] ;
				$data_array["grade"] = $serch_grade[$grade_num];
